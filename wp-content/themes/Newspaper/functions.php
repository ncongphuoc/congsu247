						    ...
						    <div class="td-skin-container td-skin-content">
							    <div class="td-demos-list">
								    ...
								    foreach (td_global::$demo_list as $demo_id => $stack_params) {
									    ...
									    ?>
									    <div class="td-set-theme-style"><a target="_blank" href="<?php echo td_global::$demo_list[$demo_id]['demo_url'] ?>" class="td-set-theme-style-link" data-img-url="http://demo.tagdiv.com/demos_popup/newspaper/large/<?php echo $demo_id; ?>.jpg"><img width="140" height="100" alt="" title="" src="http://demo.tagdiv.com/demos_popup/newspaper/small/<?php echo $demo_id; ?>.jpg" /></a></div>
								    <?php } ?>
...
function get_keyword($post_id)
{
	global $wpdb;
	$arr_resutl = $wpdb->get_row( "SELECT * FROM wp_word_post WHERE post_id = " . $post_id );
	$html = '';
	if(!empty($arr_resutl->arr_word)){
		$STATIC_URL = get_site_url() . "/?s=";
		$url = get_site_url() . "/?s=";
		$html = '<ul class="td-tags td-post-small-box clearfix">';
		$html .= '<li><span>Từ khóa</span></li>';
		$arr_keyword = explode(",",$arr_resutl->arr_word);
		foreach($arr_keyword as $keyword){
			$url = $STATIC_URL . implode("+",explode(" ",$keyword));
			$html .= '<li><a href="' . $url . '">' . $keyword . '</a></li>';
		}
		$html .= "</ul>";
	}
	echo($html) ;
}
add_action("get_keyword",get_keyword,10);
